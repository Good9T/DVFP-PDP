...
class MixedScoreAttention(nn.Module):
    def __init__(self, **model_params):
        ...
        self.sqrt_qkv_dim = int(self.model_params['sqrt_qkv_dim'])
        ms_hidden_dim = model_params['ms_hidden_dim']
        mix1_init = model_params['ms_layer1_init']
        mix2_init = model_params['ms_layer2_init']
        mix1_weight = torch.distributions.Uniform(low=-mix1_init, high=mix1_init).sample(sample_shape=(self.head_num, 2, ms_hidden_dim))
        mix1_bias = torch.distributions.Uniform(low=-mix1_init, high=mix1_init).sample(sample_shape=(self.head_num, ms_hidden_dim))
        self.mix1_weight = nn.Parameter(mix1_weight)
        self.mix1_bias = nn.Parameter(mix1_bias)

        mix2_weight = torch.distributions.Uniform(low=-mix2_init, high=mix2_init).sample(sample_shape=(self.head_num, ms_hidden_dim, 1))
        mix2_bias = torch.distributions.Uniform(low=-mix2_init, high=mix2_init).sample(sample_shape=(self.head_num, 1))
    ...
    def forward(self, q, k, v, problem):
        ...
        dot_product_score = dot_product.unsqueeze(-1) / self.sqrt_qkv_dim
        problem_score = (problem[:, None, :, :].expand(batch_size, self.head_num, row_size, col_size)).unsqueeze(-1)
        ...
        out = out.reshape(batch_size, row_size, self.head_num * self.qkv_dim)
        return out


def _get_encoding(encoded_nodes, node_index_to_pick):
    # encoded_node.shape: (batch, problem, embedding)
