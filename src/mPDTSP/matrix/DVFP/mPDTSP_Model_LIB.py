...
class MixedScoreAttention(nn.Module):
    def __init__(self, **model_params):
        ...
        mix1_weight = torch.distributions.Uniform(low=-mix1_init, high=mix1_init).sample(
            sample_shape=(head_num, 3, ms_hidden_dim))
        mix1_bias = torch.distributions.Uniform(low=-mix1_init, high=mix1_init).sample(
            sample_shape=(head_num, ms_hidden_dim))
        self.mix1_weight = nn.Parameter(mix1_weight)
        self.mix1_bias = nn.Parameter(mix1_bias)

        mix2_weight = torch.distributions.Uniform(low=-mix2_init, high=mix2_init).sample(
            sample_shape=(head_num, ms_hidden_dim, 1))
    ...
    def forward(self, q, k, v, problem):
        ...
        mixed_score = mix_score2.transpose(1, 2).squeeze(4)
        weights = nn.Softmax(dim=3)(mixed_score)
        out = torch.matmul(weights, v)
        out = out.transpose(1, 2).reshape(batch_size, row_size, head_num * qkv_dim)
        return out
