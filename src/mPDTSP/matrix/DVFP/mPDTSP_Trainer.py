...
from mPDTSP_Env import mPDTSPMatrixEnv as Env
from mPDTSP_Model import mPDTSPModel as Model
...
class mPDTSPTrainer:
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, epochs + 1):
            ...
            self.result_log.append('train_loss_dual', epoch, loss2)
            ...
            if epoch > 1:
                ...
                util_save_log_image_with_label(image_prefix, self.trainer_params['logging']['log_image_params_2'],
                                               self.result_log, labels=['train_loss_dual'])
    ...
    def _train_one_epoch(self, epoch):
        ...
        while episode < train_episode_num:
            ...
            if epoch == self.start_epoch:
                loop_cnt += 1
                if loop_cnt <= 10:
                    self.logger.info('Epoch {:3d}: Train {:3d}/{:3d}({:1.1f}%) Score: {:.4f}, Loss: {:.4f}, Loss_primal: {:.4f}, Loss_dual: {:.4f}'
                                     .format(epoch, episode, train_episode_num, 100. * episode / train_episode_num,
                                             score.avg, loss.avg, loss1.avg, loss2.avg))

        self.logger.info('Epoch {:3d}: Train ({:3.0f}%) Score: {:.4f}, Loss: {:.4f}, Loss_primal: {:.4f}, Loss_dual: {:.4f}'
    ...
    def _train_one_batch(self, batch_size):
        ...
        # dual problem
        self.env.load_dual_problems()
        D_reset_state, _, _ = self.env.reset()
        self.model.pre_forward(D_reset_state)
        D_prob_list = torch.zeros(batch_size, self.env.pomo_size, 0)
        D_state, D_reward, D_done = self.env.pre_step()
        while not D_done:
            D_selected, D_prob = self.model(D_state)
            D_state, D_reward, D_done = self.env.step(D_selected)
            D_prob_list = torch.cat((D_prob_list, D_prob[:, :, None]), dim=2)

        D_log_prob = D_prob_list.log().sum(dim=2)
        combined_reward = torch.cat([reward, D_reward], dim=1)
        # (batch , 2*pomo)
        ...
        alpha = self.trainer_params.get('primal_dual_weight', 0.5)

        advantage = combined_advantage[:, :self.env.pomo_size]
        dual_advantage = combined_advantage[:, self.env.pomo_size:]

        # loss
        primal_loss = -advantage * log_prob
        D_loss = -dual_advantage * D_log_prob
        # (batch, pomo)
        loss_mean = primal_loss.mean()
        D_loss_mean = D_loss.mean()
        final_loss_mean = alpha * loss_mean + (1 - alpha) * D_loss_mean
        ...
        return score_mean.item(), final_loss_mean.item(), loss_mean.item(), D_loss_mean.item()