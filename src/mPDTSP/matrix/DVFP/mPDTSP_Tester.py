...
class mPDTSPTester:
    ...
    def _test_one_batch(self, batch_size):
        ...
        with torch.no_grad():
            ...
            while not D_done:
                D_selected, _ = self.model(D_state)
                D_state, D_reward, D_done = self.env.step(D_selected)

            D_aug_reward = D_reward.reshape(aug_factor, batch_size, self.env.pomo_size)
            # shape: (augmentation, batch, pomo)
            D_max_pomo_reward, _ = D_aug_reward.max(dim=2)
            # shape: (augmentation, batch)
            D_batch_reward, _ = D_max_pomo_reward.max(dim=0)
            # shape: (batch,)
            max_batch_reward = torch.max(batch_reward, D_batch_reward)
            avg_reward_final = -max_batch_reward.float().mean()

            return avg_reward_pre.item(), avg_reward_final.item()